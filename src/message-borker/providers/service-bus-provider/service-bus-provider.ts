...
import { HandleMessageArgs, TopicListener } from '../../../../libs/message-broker/src/models/topic-listener.interface';
...
export class ServiceBusService implements MessageBrokerProvider {
  ...
  listenToTopic(listener: TopicListener, options: ListenToTopicOptions = {destructiveRead: true}): SubscriptionClose {
    this.logger.info(`[ServiceBusService:listenToTopic] topic: %s, subscription: %s`, { messageParams: [listener.topicName, listener.subscriptionName] });
    ...
    this.logger.info('[ServiceBusService:listenToTopic] Receiver mode: %s ', {
    ...
    this.logger.trace('[ServiceBusService:listenToTopic] autoCompleteMessages : %s ', {
  ...
  async postMessageToTopic(topicName: string, payload: string, options: PostMessageOptions): Promise<void> {
    this.logger.trace('[ServiceBusService:postMessageToTopic] topic: %s, messageId: %s, correlationId: %s.', {
    ...
    try {
      ...
      this.logger.trace(`[ServiceBusService:postMessageToTopic] converted message object : %o .`, { messageParams: [serviceBusMessage] });
      await sender.sendMessages(serviceBusMessage);
      this.logger.trace(`[ServiceBusService:postMessageToTopic] sendMessages done messageId: %s.`, { messageParams: [options.messageId] });
    } catch (error) {
  ...
  protected createReceiverOptionsAdapter(options: ListenToTopicOptions): ServiceBusReceiverOptions {
    return {
      receiveMode: options.destructiveRead == undefined ? options.destructiveRead : options.destructiveRead ? 'peekLock' : 'receiveAndDelete',
    };
  }
  ...
  protected processMessageAdapter(ctx: TopicListener, handleMessage: TopicListener['handleMessage']): MessageHandlers['processMessage'] {
    return (messageArgs: ServiceBusReceivedMessage): Promise<void> => {
      ...
      this.logger.trace('[ServiceBusService:processMessageAdapter] messageId: %s .', { messageParams: [messageArgs.messageId] });
      ...
      return handleMessage
        ...
        .then(() => {
          this.logger.trace('[ServiceBusService:processMessageAdapter] message completed: %s .', { messageParams: [messageArgs.body] });
  ...
  isTopicListenerConnected(topicListener: TopicListener): boolean {
    this.logger.trace('[ServiceBusService:isTopicListenerConnected] Checking connection to topic: %s, subscription: %s', {
